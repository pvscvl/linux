...
function show-help() {
	printf '%s\n' \
		"pps-bash.sh ${VERSION}" \
		"" \
		"Post-installation script for Debian and Ubuntu machines (VM, LXC, bare metal)." \
		"" \
		"Usage:" \
		'  bash -c "$(wget -qLO - https://raw.githubusercontent.com/pvscvl/linux/main/pps-bash.sh)" _ [--help]' \
		"" \
		"Options:" \
		"  -h, --help          show this help" \
		"" \
		"Environment:" \
		"  ZABBIX_VERSION      Zabbix release to install the agents from (default: 7.0 LTS)." \
		"                      Versions without a 'release' repo path are only known for 7.0;" \
		"                      newer ones (for example 8.0) are looked up under <version>/release/." \
		"" \
		"Behavior:" \
		"  - Installs the base packages (curl, wget, unzip, ncdu, ripgrep, net-tools)." \
		"  - Checks for gum at the start and installs it (distro repo, then the charm apt repo)." \
		"  - With gum: shows a multi-select list of all applicable steps; Enter runs the selected ones." \
		"  - Without gum (not installable, or no terminal): asks yes/no for every step, as before." \
		"" \
		"Steps (only the applicable ones are listed in the gum menu):" \
		"  root login: set password" \
		"  sshd_config: permit root login" \
		"  ssh: copy public keys (only if the key server is reachable)" \
		"  .bashrc: modify (backup is created first)" \
		"  pfetch, ack, Midnight Commander: install" \
		"  qemu-guest-agent: install (KVM guests)" \
		"  linux-virtual-packages: install (Ubuntu on KVM)" \
		"  KVP daemon bug: apply workaround (Ubuntu 20.04/22.04 on KVM)" \
		"  zabbix-agent / zabbix-agent2: install and configure" \
		"  install updates, install dist-upgrades, reboot" \
		"" \
		"Zabbix supported systems: Debian 10-13, Ubuntu 20.04, 22.04, 24.04, 26.04." \
		"Timezone is set to Europe/Berlin if it differs. Root privileges are required."
}
...
function install-gum() {
	...
	DEBIAN_FRONTEND=noninteractive apt-get install -y gum &>/dev/null
	if command -v gum &>/dev/null; then
		return 0
	fi
	mkdir -p /etc/apt/keyrings
	if ! curl -fsSL https://repo.charm.sh/apt/gpg.key | gpg --dearmor --yes -o /etc/apt/keyrings/charm.gpg &>/dev/null; then
		return 1
	fi
...
function select-steps() {
	...
	add-step "rootpw" "root login: set password"
	add-step "sshd" "sshd_config: permit root login"
	if [ "$WEBSITE_AVAILABLE" = true ]; then
		add-step "sshkeys" "ssh: copy public keys"
	fi
	add-step "bashrc" ".bashrc: modify"
	if [ ! -x "$(command -v pfetch)" ]; then
		add-step "pfetch" "pfetch: install"
	fi
	if [ ! -x "$(command -v ack)" ]; then
		add-step "ack" "ack: install"
	fi
	if [ ! -x "$(command -v mc)" ]; then
		add-step "mc" "Midnight Commander: install"
	fi
	if [[ $detected_env == "kvm" && ! -x "$(command -v qemu-ga)" ]]; then
		add-step "qemu" "qemu-guest-agent: install"
	fi
	if [[ $detected_os == "ubuntu" && $detected_env == "kvm" ]] && ! dpkg -s linux-virtual &>/dev/null; then
		add-step "linuxvirt" "linux-virtual-packages: install"
	fi
	if [[ -f /etc/systemd/system/multi-user.target.wants/hv-kvp-daemon.service && $detected_env == "kvm" && $detected_os == "ubuntu" && ($detected_version == "22.04" || $detected_version == "20.04") ]] && ! grep -q "^After=systemd-remount-fs.service" /etc/systemd/system/multi-user.target.wants/hv-kvp-daemon.service; then
		add-step "kvp" "KVP daemon bug: apply workaround"
	fi
	if ${ZABBIX_SUPPORTED}; then
		add-step "zabbixagent" "zabbix-agent: install"
		add-step "zabbixagent2" "zabbix-agent2: install"
	fi
	add-step "updates" "$hostsys: install updates"
	...
	while IFS= read -r STEP_LINE; do
		for STEP_INDEX in "${!STEP_LABELS[@]}"; do
			if [[ "${STEP_LABELS[${STEP_INDEX}]}" == "${STEP_LINE}" ]]; then
				GUM_SELECTED+=" ${STEP_KEYS[${STEP_INDEX}]}"
			fi
		done
	done <<< "${STEP_CHOICE}"
}

function prepare-zabbix() {
	local ZABBIX_DIR
	local ZABBIX_FILE
	local ZABBIX_URL

	if ${ZABBIX_READY}; then
		return 0
	fi

	case "${ZABBIX_VERSION}" in
		7.0)
			ZABBIX_DIR="${ZABBIX_VERSION}/${detected_os}"
			;;
		*)
			ZABBIX_DIR="${ZABBIX_VERSION}/release/${detected_os}"
			;;
	esac
	ZABBIX_FILE="zabbix-release_latest_${ZABBIX_VERSION}+${detected_os}${detected_version}_all.deb"
	ZABBIX_URL="https://repo.zabbix.com/zabbix/${ZABBIX_DIR}/pool/main/z/zabbix-release/${ZABBIX_FILE}"

	rm -f "/tmp/${ZABBIX_FILE}"
	if wget -q -O "/tmp/${ZABBIX_FILE}" "${ZABBIX_URL}" && DEBIAN_FRONTEND=noninteractive dpkg --force-confnew -i "/tmp/${ZABBIX_FILE}" &>/dev/null; then
		apt update &>/dev/null
		ZABBIX_READY=true
	fi
	rm -f "/tmp/${ZABBIX_FILE}"
...
if ! command -v gum &>/dev/null; then
	msg_info "${BOLD}gum:${DEFAULT} installing"
	if install-gum; then
		msg_ok "${BOLD}gum:${DEFAULT} installed"
	else
		msg_lno "${BOLD}gum:${DEFAULT} not installable, using the classic prompts"
	fi
	echo ""
fi
...
case "${detected_os}-${detected_version}" in
	debian-10|debian-11|debian-12|debian-13|ubuntu-20.04|ubuntu-22.04|ubuntu-24.04|ubuntu-26.04)
		ZABBIX_SUPPORTED=true
		;;
	*)
		ZABBIX_SUPPORTED=false
		;;
...
if ${ZABBIX_SUPPORTED}; then
	...
	if [[ $prompt =~ ^[Yy][Ee]?[Ss]?|[Jj][Aa]?$ ]] ; then
		...
		if prepare-zabbix; then
			...
			msg_lno "${BOLD}zabbix-agent:${DEFAULT} zabbix ${ZABBIX_VERSION} repository could not be set up for $detected_os $detected_version"
		fi
	else
...
if ${ZABBIX_SUPPORTED}; then
	...
	if [[ $prompt =~ ^[Yy][Ee]?[Ss]?|[Jj][Aa]?$ ]] ; then
		...
		if prepare-zabbix; then
			apt update &>/dev/null
			DEBIAN_FRONTEND=noninteractive apt-get install zabbix-agent2 -y &>/dev/null
			DEBIAN_FRONTEND=noninteractive apt-get install zabbix-agent2-plugin-mongodb -y &>/dev/null
			...
			msg_lno "${BOLD}zabbix-agent2:${DEFAULT} zabbix ${ZABBIX_VERSION} repository could not be set up for $detected_os $detected_version"
		fi
	else
